...
a2enmod ssl # enabling ssl mod
a2enmod rewrite # enabling alias mode to force https redirection
a2ensite site-ssl.conf > /dev/null # enabling https config
a2ensite site.conf > /dev/null # enabling http config

service apache2 restart # restarting apache to apply configuration
